...
@Injectable()
export class DeleteUserInputAction implements BaseAction {
  // Logger
  private readonly logger = new Logger(DeleteUserInputAction.name);
  ...
  async exec(messageData: IMessageData): Promise<void> {
    ...
    try {
      link = await this.databaseService.deleteLink(message, chatId);
    } catch (error) {
    ...
    await this.bot.sendMessage(
      chatId,
      `Your URL ${link.url} (${link.shortCode}) is now deleted.`,
    );
  }
}
